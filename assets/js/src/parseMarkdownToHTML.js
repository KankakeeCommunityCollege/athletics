...
function replacer(match, p1) {
  if ( p1 === '' ) { // Filters out the '__' (double underscore) matches.
    ...
    for (var el in inlineMarkdownObject) {
      if (inlineMarkdownObject.hasOwnProperty(el)) {
        match = match.replace(inlineMarkdownObject[el], '<' + el + '>$1</' + el + '>' );
      }
    }
    return match;
  }
...
function createInlineElements(string) {
  for (var el in inlineMarkdownObject) {
    if (inlineMarkdownObject.hasOwnProperty(el)) {
      string = string.replace(inlineMarkdownObject[el], replacer);
    }
  }
  return string;
}
